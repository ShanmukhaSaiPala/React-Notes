...
const AddEditNotes = ({ onClose, noteData, type, getAllNotes }) => {
  ...
  const editNote = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_REACT_BACKEND_BASEURL}/api/note/edit/` + noteId,
        { title, content, tags },
  ...
  const addNewNote = async () => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_REACT_BACKEND_BASEURL}/api/note/add`,
        { title, content, tags },
        { withCredentials: true }
      );
  ...
  return (
    <div className="relative">
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          type="text"
          className="text-2xl bg-base-100  rounded-lg text-base-content p-2 outline-none"
      ...
      <div className="flex flex-col gap-2 mt-4">
        ...
        <textarea
          type="text"
          className="text-sm bg-base-100 text-base-content outline-none  p-2 rounded-lg"
          placeholder="Content..."
          rows={10}
          value={content}
